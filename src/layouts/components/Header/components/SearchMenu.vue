<template>
    <div class="menu-search-dialog">
        <i :class="'iconfont icon-sousuo'" class="toolBat-icon" @click="handleOpen"></i>
...
const handleOpen = () => {
    ...
    nextTick(() => menuInputRef.value.focus());
};
...
const filterNodeMethod = (queryString: string) => {
    return (restaurant: Menu.MenuOptions) => {
        return (
            restaurant.path.toLowerCase().indexOf(queryString) !== -1 ||
            restaurant.meta.title.toLowerCase().indexOf(queryString) !== -1
        );
    };
};
